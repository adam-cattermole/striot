...
streamFilter ff s = filter (\(Event m t v) -> case v of
                                                  Just val -> ff val
...
streamMap fm s = map (\(Event m t v) -> case v of
                                            Just val -> Event m t (Just (fm val))
                                            Nothing  -> Event m t Nothing       ) -- allow timestamped events to pass through for time-based windowing
                     s
...
streamWindow fwm s = mapWindowId (fwm s)
           where getVals :: Stream alpha -> [alpha]
                 ...
                 mapWindowId (x:xs) =
                     case x of
                         Event m t _ : _ -> Event m         t       (Just (getVals x)) : mapWindowId xs
                         []              -> Event Nothing   Nothing (Just [])          : mapWindowId xs
...
streamJoin _                                []                               = []
streamJoin []                               _                                = []
streamJoin    ((Event m1  t1 (Just v1)):r1)    ((Event _  _  (Just v2)):r2) = (Event m1 t1 (Just(v1,v2))):(streamJoin r1 r2)
...
streamScan _  _   []                       = []
streamScan mf acc (Event m t (Just v):r) = Event m t (Just newacc):streamScan mf newacc r where newacc = mf acc v
streamScan mf acc (Event m t Nothing :r) = Event m t Nothing      :streamScan mf acc    r -- allow events without data to pass through

instance Arbitrary a => Arbitrary (Event a) where
    arbitrary = do
        i <- arbitrary
        return $ Event Nothing Nothing (Just i)
...
s1 :: Stream Int
s1 = [(Event Nothing (Just (addUTCTime i (read "2013-01-01 00:00:00"))) (Just 999))|i<-[0..]]

s2 :: Stream Int
s2 = [Event Nothing (Just (addUTCTime i (read "2013-01-01 00:00:00"))) Nothing |i<-[0..]]

s3 :: Stream Int
s3 = streamMerge [s1,s2]

s4 :: Stream Int
s4 = [Event Nothing Nothing (Just i)|i<-[0..]]

s5 :: Stream Int
s5 = streamMerge [s2,s4]

s6 :: Stream Int
s6 = [Event Nothing Nothing (Just i)|i<-[100..]]
