...
journeyChanges (Event _ _ _ (Just val):r) = streamFilterAcc (\acc h -> if snd h == snd acc then acc else h) val (\h acc -> snd h /= snd acc) r

--- removes consecutive repeated values from a stream, leaving only the changes
changes :: Eq alpha => Stream alpha -> Stream alpha
changes (e@(Event _ _ _ (Just val)):r) = e : streamFilterAcc (\_ h -> h) val (/=) r
...
tripTimes [] = []
tripTimes (e@(Event i m _ v):t) = [Event i m (fmap dropoffDatetime v) v] : tripTimes t
...
-- simulateData :: String -> IO (Stream Trip)
-- simulateData fileName = do
--     contents <- readFile fileName
--     let stream = tripSource contents
--     go stream
--         where
--             go :: Stream Trip -> IO (Stream Trip)
--             go (x:xs) = unsafeInterleaveIO $ do
--                 let getTime e = fromJust . time $ e 
--                     diff      = diffUTCTime (getTime $ head xs) (getTime x)
