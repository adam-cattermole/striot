...
sink1 s = do
    now <- getCurrentTime
    let fileName = "data/migrate_" ++ (formatTime defaultTimeLocale "%d-%m-%Y_%H%M%S" now) ++ ".txt"
...
sinkLatencyFile stream = do
    now <- getCurrentTime
    let fileName = "output/migrate_" ++ (formatTime defaultTimeLocale "%d-%m-%Y_%H%M%S" now) ++ ".txt"
